...
class Wrangler:
    ...
    def bin_and_split_data(self, xdata, ydata, test_size=.15):
        """
        ...
        for self.ifold in range(self.n_splits):
            ...
            X_train_all, X_test_all, y_train, y_test = train_test_split(xdata_binned,ydata_binned,stratify=ydata_binned, test_size=test_size)
...
class Interpreter:
    ...
    def plot_hyperplane(self, subtitle='', stim_time=[0, 250], significance_testing_pair=[], sig_y=1,
        ...
        if len(significance_testing_pair) > 0:
            ...
            for score_i,score_ii in significance_testing_pair:
                ...
                for i, t in enumerate(np.arange(len(self.t))[self.t > 0]):
                    # one-sided paired ttest
                    _, p[i] = sista.ttest_rel(
                        a = np.mean(self.confidence_scores,2)[:, t, score_i], b=np.mean(self.confidence_scores,2)[:, t, score_ii],alternative='less')

                # Use Benjamini-Hochberg procedure for multiple comparisons, defaults to FDR of .05
                _, corrected_p, _, _ = multipletests(p, method='fdr_bh')
                sig05 = corrected_p < .05

                plt.scatter(self.t[self.t > 0][sig05]-10, np.ones(sum(sig05))*(sig_y-offset),
                            marker='s', s=33, c=color, label='p < .05')
                sig_timepoints = self.t[self.t>0][sig05]
                print(f'Significant timepoints: {sig_timepoints}')
                offset += .15
                color = 'orange'
        ...
        plt.text(label_text_x,label_text_ys[0],f'Predicted\n{self.labels[0]}',fontsize=16,ha='center',va='top')
        plt.text(label_text_x,label_text_ys[1],f'Predicted\n{self.labels[-1]}',fontsize=16,ha='center',va='bottom')
        plt.text(stim_label_xy[0],stim_label_xy[1],'Stim',fontsize=14,ha='center',c='white')
        plt.arrow(label_text_x,arrow_ys[0],0,-1,head_width=45, head_length=.25,color='k', width=5)
        plt.arrow(label_text_x,arrow_ys[1],0,1, head_width=45, head_length=.25,color='k', width=5)
