...
instruct_text = [(
    ...
    'An array of colored circles will appear.\n'
    'Remember the colors and their locations as best you can.\n'
    'Ignore the non-target gray items. You will not be tested on these.\n'
    'After a short delay, another color will reappear.\n'
    'If it has the SAME color as the item in its location before, press the "S" key.\n'
...
class Cannonball01_color(template.BaseExperiment):
    ...
    def realtime_eyetracking(self,wait_time,sampling_rate=.01):
...
#            reject=False
    ...
    def make_block(self, number_of_trials_per_block = None):
        ...
        set_size_block = np.tile(self.set_sizes, number_of_trials_per_block//2)
        change_block = np.tile([0,0,1,1], number_of_trials_per_block//4)

        block = []
        for set_size,change in zip(set_size_block,change_block):
            block.append(self.make_trial(change,set_size))

        block = np.random.permutation(block)

        return block
        
    def make_trial(self, change, set_size):
    ...
    def display_start_block_screen(self):
        ...
        self.display_text_screen(text=f'Press s to continue',keyList=['s'])
    ...
    def display_fixation(self,wait_time = None, text = None, keyList = None, realtime_eyetracking = False, trial = None):
        ...
        if realtime_eyetracking:
            reject = self.realtime_eyetracking(wait_time=wait_time)
            return reject    
        else:
            if keyList:
    ...
    def display_stimuli(self, trial, locations, colors, realtime_eyetracking=False):
        ...
        for i in range(len(colors)):
            self.draw_stim(colors[i],locations[i])
        for i in range(3-len(colors)):
            distractor.pos = locations[2-i]
            distractor.draw()
        ...
        if realtime_eyetracking:  
            reject = self.realtime_eyetracking(wait_time=self.sample_time)
            return reject
        else:
            psychopy.core.wait(self.sample_time)

    def display_test(self, locations, test_color):
        """Displays the test array. A helper function for self.run_trial.

        Parameters:
        locations -- Locations of items.
        ...
        self.draw_stim(color=test_color,pos=locations[0])
    ...
    def run_trial(self, trial, block_num, trial_num, realtime_eyetracking=False):
        ...
        self.display_test(trial['locations'], trial['test_color'])
    ...
    def run_makeup_block(self,block_num):

            number_of_trials_per_block = copy.copy(self.color_rej_counter)
            self.color_rej_counter=0
            
            self.rejection_tracker = np.zeros(5)
            block = self.make_block(number_of_trials_per_block=number_of_trials_per_block)
    ...
    def run(self):
        ...
        while prac == ['y']: 
            ...
            block = self.make_block(number_of_trials_per_block=12)
            acc = []
        ...
        Makeup Blocks
        """        
        while self.color_rej_counter > 15:
            block_num += 1
            self.run_makeup_block(block_num=block_num)
