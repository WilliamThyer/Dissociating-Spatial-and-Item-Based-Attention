...
number_of_trials_per_block = 12 # must be divisible by 12
number_of_blocks = 4
...
class Cannonball02(template.BaseExperiment):
    ...
    def __init__(self, number_of_trials_per_block=number_of_trials_per_block,
                 ...
                 stim_size=stim_size, letters=letters, keys = keys,
    ...
    def make_block(self, block_num, number_of_trials_per_block = None, block_condition = None):
        ...
        for itrial in range(number_of_trials_per_block):
            block.append(self.make_trial(block_dict,change_block[itrial], quads[itrial]))
        
        block = np.random.permutation(block)

        return block
        
    def make_trial(self, block_dict, change, quads):
        ...
        if change == 0:
            ...
            if block_dict['block_condition'] is not 'L1F':
                test_item = self.digits[np.random.choice(np.setdiff1d(self.stim_idx,digit_idx[0]))][0]
            else:
                test_item =  self.letters[np.random.choice(np.setdiff1d(self.stim_idx,letter_idx[0]))][0]
            cresp = 'd'
    ...
    def display_test(self, trial):
        ...
        self.draw_stim(trial['test_item'],trial['test_location'])
    ...
    def run_makeup_block(self,block_condition,block_num,block_name):
            ...
            for trial_num, trial in enumerate(block):
                if trial_num == 0:
                    self.display_start_block_screen(block_name)
                if trial_num % 5 == 0:
    ...
    def run(self):
        ...
        for block_con in self.block_conditions:
            while self.rej_counter[block_con] > 25:
                block_num += 1
                self.run_makeup_block(block_condition=block_con,block_num=block_num,block_name=trial['block_name'])

        """
        L1F Blocks
        """
        for _ in range(1):# check number of blocks needed
            block_num += 1
