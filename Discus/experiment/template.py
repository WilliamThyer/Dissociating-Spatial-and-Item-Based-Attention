...
class BaseExperiment:
    ...
    def save_experiment_info(self, filename=None):
        ...
        if filename is None:
            ...
            #             self.experiment_info['Subject Number'].zfill(2) +
            #             + '_' + str(self.experiment_info['Run']) + '_info')
            filename = (f'{self.experiment_name}_{self.experiment_info["Subject Number"].zfill(2)}_{str(self.experiment_info["Run"])}')
        elif filename[-5:] == ext:
            filename = filename[:-5]
...
#                           self.experiment_info['Subject Number'].zfill(3) + '.pickle')
        pickle_filename = (f'{self.experiment_name}_{self.experiment_info["Subject Number"].zfill(2)}_{str(self.experiment_info["Run"])}.pickle')
