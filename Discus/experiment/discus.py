...
class Discus(template.BaseExperiment):
    ...
    def __init__(self, number_of_trials_per_block=number_of_trials_per_block,
        ...
        self.quints_dict = {0: (54, 126), 1: (126, 198), 2: (
            198, 270), 3: (270, 342), 4: (342, 54)}
...
#            reject=False
    ...
    def make_run(self, practice=False):
        ...
        for iblock, block in enumerate(blocks):
            for itrial, trial in enumerate(block):
                run.append(self.make_trial(iblock, itrial,
                                           conditiont[trial], loc_1t[trial], loc_2t[trial]))
    ...
    def make_makeup_block(self, block_num, trial_num):
        ...
        for trial in self.rej_counter:
            ...
            run.append(self.make_trial(
                block_num, trial_num, condition, loc1, loc2))
            trial_num += 1
    ...
    def make_practice_blocks(self, condition):
        ...
        for cond in self.conditions[:3]:
            ...
            block_trials = np.random.permutation(
                np.concatenate([ss2_trials[:5], other_trials[:5]]))
    ...
    def make_blocks(self, condition):
        ...
        ss2a, ss2b, ss2c = np.split(
            np.random.permutation(int_idx[idx_ss2]), [16, 32])

        # counterbalance order of blocks
        sub_shift = (int(
            self.experiment_info['Subject Number']) + int(self.experiment_info['Run'])) % 3
        ...
        for ss2_trials, cond in zip([ss2a, ss2b, ss2c], block_conditions):
            idx = condition == cond
            block_trials = np.random.permutation(
                np.concatenate([int_idx[idx], ss2_trials]))
    ...
    def generate_locations(self, quints):
        ...
        while len(locs) < 2:
            ...
            a = angle*math.pi / 180
            attempt = (self.distance_from_fix*math.cos(a),
                       self.distance_from_fix*math.sin(a))
    ...
    def display_start_block_screen(self):
        ...
        self.display_text_screen(text='Press space to begin the next block.', keyList=[
                                 'space'], bg_color=[200, 200, 200])
    ...
    def display_stimuli(self, trial, realtime_eyetracking=False):
        ...
        if realtime_eyetracking:
            reject = self.realtime_eyetracking(
                trial, wait_time=self.sample_time)
            return reject
        else:
    ...
    def _calc_mouse_color(self, mouse_pos):
        ...
        """
        frame = np.array(self.experiment_window._getFrame()
                         )  # Uses psychopy internal function

        x_correction = self.experiment_window.size[0] / 2
        y_correction = self.experiment_window.size[1] / 2

        x = int(psychopy.tools.monitorunittools.deg2pix(
            mouse_pos[0], self.experiment_monitor) + x_correction)
    ...
    def _calc_mouse_position(self, coordinates, mouse_pos):
        ...
        Parameters:
            ...
            mouse_pos -- A position returned by mouse.getPos()
        """
        dists = [np.linalg.norm(np.array(i) - np.array(mouse_pos) / 2)
    ...
    def _response_loop(self, coordinates, wheel_rotations):
        ...
        while True:
            ...
            if px_color is not None and not np.array_equal(px_color, np.array([127, 127, 127])):
                preview_pos = self._calc_mouse_position(
                    temp_coordinates, mouse_pos)
    ...
    def get_response(self, coordinates, wheel_rotations):
        ...
        if not self.mouse:
            self.mouse = psychopy.event.Mouse(
                visible=False, win=self.experiment_window)
    ...
    def calculate_error(self, color_index, resp_color):
        ...
        row_index = np.where(
            (np.array(self.color_wheel) == resp_color).all(axis=1))[0]
    ...
    def run_trial(self, trial, realtime_eyetracking=False):
        ...
        resp_color, rt = self.get_response(
            trial['locations'][0], trial['wheel_rotation'])
        color = self.convert_color_value(resp_color)
        error, resp_index = self.calculate_error(
            trial['color_indexes'][0], color)

        true_color = self.convert_color_value(
            trial['test_item'], deconvert=True)
        colors = [self.convert_color_value(
            n, deconvert=True) for n in trial['colors']]
    ...
    def run(self):
        ...
        prac = self.display_text_screen(
            text=f'Practice block?', keyList=['y', 'n'])
